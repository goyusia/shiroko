...
type PingInput {
  PingInput(delay_ms: Int, long_mode: Int)
}
...
fn ping_command() {
  ...
  clip.command({
    ...
    use long <- clip.parameter
    PingInput(millis, long)
  })
  |> clip.arg(millis_arg)
  |> clip.opt(long_opt)
...
const long_text = "농업생산성의 제고와 농지의 합리적인 이용을 위하거나 불가피한 사정으로 발생하는 농지의 임대차와 위탁경영은 법률이 정하는 바에 의하여 인정된다.  헌법재판소의 장은 국회의 동의를 얻어 재판관중에서 대통령이 임명한다. 국가는 재해를 예방하고 그 위험으로부터 국민을 보호하기 위하여 노력하여야 한다.
...
fn text(input: PingInput) {
  case input.long_mode {
    0 -> "ping: " <> kst_now()
    _ -> long_text
  }
}

fn ping_immediate(input: PingInput, reporter: Reporter) {
  let pong = text(input)
  reporter.send(pong)
}
